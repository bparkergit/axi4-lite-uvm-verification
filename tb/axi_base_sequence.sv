    ...
    task body();
        ...
        repeat(30) begin   // 30 pairs → 60 txns
            ...
            assert(item.randomize() with {
                ...
                s_axi_awaddr[1:0] == 0;
                s_axi_wstrb dist {4'b1111 := 70, [1:14] := 30};  // mostly full
                s_axi_rready == 1'b1;
            ...
            `uvm_info("SEQ", $sformatf("WRITE: addr=0x%08h wdata=0x%08h wstrb=0x%h",
                                       item.s_axi_awaddr, item.s_axi_wdata, item.s_axi_wstrb), UVM_MEDIUM)
            ...
            `uvm_info("SEQ", $sformatf("READ:  addr=0x%08h", item.s_axi_araddr), UVM_MEDIUM)
